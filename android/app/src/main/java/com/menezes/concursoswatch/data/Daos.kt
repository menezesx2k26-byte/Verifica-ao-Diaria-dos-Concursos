...
interface AlertDao {
    ...
    @Upsert
    suspend fun upsert(items: List<AlertEntity>)

    @Query("UPDATE alerts SET unread = 0 WHERE id = :id")
    suspend fun markRead(id: Int)

    @Query("UPDATE alerts SET unread = 0")
    suspend fun markAllRead()
}
...
interface SourceHealthDao {
    ...
    @Upsert
    suspend fun upsert(items: List<SourceHealthEntity>)
}
